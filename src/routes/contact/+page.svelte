...
import Contact from './contact.svelte';
  const contacts = [{
    name:'github',
    link:"https://github.com/BeanyZoldyck",
    svg: `<svg
							fill="none"
							stroke="currentColor"
  ...
  {
    name:'linkedin',
    link:"https://linkedin.com/in/chukaez",
    svg:`<svg
						fill="none"
						stroke="currentColor"
						stroke-linecap="round"
  ...
  {
    name:"devpost",
link:"https://devpost.com/cezeoke",
...
<section class="py-20">
	<h1 class="mb-8 text-4xl font-bold text-purple-300">contact</h1>

	<div class="space-y-8">
		<div class="border-l-2 border-purple-700 pl-6">
			<p class="mb-2 text-lg text-purple-400">feel free to hit me:</p>
<Contact link="mailto:hello@example.com" name='email' svg={`<svg
					fill="none"
					stroke="currentColor"
					stroke-linecap="round"
					stroke-linejoin="round"
        ...
        {#each contacts.sort(()=>Math.random()-0.5) as contact}
           <!-- content here -->
          <Contact name={contact.name} link={contact.link} svg={contact.svg}></Contact>
        {/each}
			</div>
		</div>
	</div>
